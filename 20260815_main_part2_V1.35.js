...
(function (global) {
  ...
  /* --- 模試の混ぜ方（V1.52） ---
     3つに分けて考える。
       solved   … 全部の肢を解いた問題。**記憶で解けてしまうので測定を汚す**
       familiar … 同じ小項目は学習済みだが、この問題は初見。**本番に最も近い**
       novel    … その小項目をまだ学んでいない。**本番より難しい**

     ■ 本番モード（実力を測る）
       solved 25% … 実測した本番の再出題率（言い回し違いを含めて25%）に合わせる
       familiar 60% … 本番の主成分。ここを厚くするほど本番の手ごたえに近づく
       novel 15% … 本番にも「手を付けていない範囲」は出る。ゼロにはしない

     ■ 直前モード（成功体験）
       solved 55% ／ familiar 45% ／ novel 0%
       未習の範囲はぶつけない。ただし合格基準は本番と同じまま変えない。 */
  var EXAM_MIX = {
    real : { solved: 0.25, familiar: 0.60, novel: 0.15 },
    final: { solved: 0.55, familiar: 0.45, novel: 0.00 }
  };
