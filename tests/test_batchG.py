...
# 期待する並び順（35件・ラベル）。V2.39：忘却曲線④を削除（t08と重複）・
# ★ノート直後に★5段階の話2件（t36/t37）を追加
ORDER = [
...
with sync_playwright() as p:
    ...
    ok("一言欄は35件", len(tips) == 35, str(len(tips)))
    ...
    ok("並び順が動線どおり（35件・完全一致）", got == ORDER,
       "\n     got=%s\n     exp=%s" % (got, ORDER))
    ok("1件目は使い方（本日の復習）から始まる", got[0] == "本日の復習", got[0])
    ok("最後の4件は製作者から",
       all(x.startswith("製作者から") for x in got[-4:]), str(got[-4:]))
    ok("製作者からは末尾にしか無い",
       [i for i, x in enumerate(got) if x.startswith("製作者から")] == [31, 32, 33, 34],
       str([i for i, x in enumerate(got) if x.startswith("製作者から")]))
    ok("評価4ボタンは連続して5〜8件目",
       got[4:8] == ["評価ボタン：難しい", "評価ボタン：普通", "評価ボタン：簡単", "評価ボタン：マスター"],
       str(got[4:8]))
    ok("忘却曲線①〜③が連続している（④はV2.39で削除）", got[8:11] == ["エビングハウスの忘却曲線 ①",
       "エビングハウスの忘却曲線 ②", "エビングハウスの忘却曲線 ③"], str(got[8:11]))
    ok("ポモドーロ①〜④が連続している",
       all(got[23 + i] == "ポモドーロ勉強法 " + c for i, c in enumerate("①②③④")), str(got[23:27]))
    ok("モード紹介はホームの並び順（ランダム→ノック→力試し）",
       got[11:14] == ["ランダムモード", "テーマ別 弱点ノック", "力試しモード"], str(got[11:14]))
    ...
    seq = pg.evaluate("""async () => {
        ...
        return out; }""")
    ok("画面上も1件目から順に出る", seq[0][0] == "本日の復習" and seq[0][1] == "1 / 35",
       json.dumps(seq, ensure_ascii=False))
    ok("5件目は「評価ボタン：難しい」", seq[4][0] == "評価ボタン：難しい" and seq[4][1] == "5 / 35",
