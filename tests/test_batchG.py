...
SPEC = {
 "評価ボタン：難しい": ("10分後に再出題",
  "いつ押しても必ず10分後に再出題されます。何度押しても10分後のままで、間隔が伸びることはありません。"
  "根拠が言えないなら、正解していても迷わずこれ。"),
 "評価ボタン：普通": ("1時間後 →翌日 →1週間に再出題",
  "初見なら1時間後。10分後か1時間後の段階なら翌日出題。そこから先は"
...
with sync_playwright() as p:
    ...
    ok("実装と一致：難しい＝常に10分後", real["h1"] == "10分後" and real["h2"] == "10分後", json.dumps(real, ensure_ascii=False))
