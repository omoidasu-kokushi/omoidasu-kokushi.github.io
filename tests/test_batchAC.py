...
ok("sw CACHE_NAME が v1.41.0 以降",
   bool(_c) and tuple(int(x) for x in _c.groups()) >= (1, 41, 0),
...
ok("版の刻印が2箇所とも V1.49", idx.count("Omoidasu_V1.52") >= 2)
...
for f in ["styles.css", "questions.js", "storage.js", "scheduler.js", "drive.js"]:
    ok("%s の版が index と sw で一致" % f, _idxq.get(f) == _swq.get(f) and _idxq.get(f) == "1.52",
