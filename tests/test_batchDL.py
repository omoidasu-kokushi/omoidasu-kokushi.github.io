#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""バッチDL：出題量設定（V2.44 → V2.69 → V2.70で作り直し）
...
V2.70（利用者裁定）で、そのセグメント自体も無くした。
押す場所が2段階（選ぶ→動かす）になり、選ばれていないほうは消えていたため。
いまは **スライダー2本を常に見せ、触ったほうが終了条件になる**。
効いているのは常に1つだけ（「共存させない」は変えていない）。

観点は消さずに置き換える。
  ・ポモドーロのON/OFF   → 触ったほうが効く（test_qty_mode.py が本体）
...
ok("代わりにスライダー2本が並んでいる（V2.70）",
   'id="qty-mode"' not in html and 'data-qmode="time"' in html and 'data-qmode="count"' in html)
...
ok("時間スライダーもある（V2.70は5段：5/10/25/45/60）",
   'id="time-range"' in html and 'max="4"' in html and "<em>60分</em>" in html)
...
with sync_playwright() as p:
    ...
    r = pg.evaluate("""async () => {
      ...
      /* V2.70：セグメントは無い。スライダーを触ればそちらが効く。 */
      ...
      /* 時間のスライダーを触ると、そちらへ戻り保存も変わる */
      const trg = document.getElementById('time-range');
      trg.value = '2';
