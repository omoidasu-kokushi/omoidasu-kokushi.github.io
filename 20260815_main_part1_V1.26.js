...
(function (global) {
  ...
  function boot() {
    ...
    return S.ensureInitialized()
      ...
      .then(function () {
        ...
        /* --- 起動時の自動同期（V1.39） ---
           前回から1時間以内ならトークンが生きている。その間だけは
           押さずに合わせられる。切れていれば黙って何もしない
           （起動のたびにログインを迫るのが一番の離脱要因になる）。
           描画は待たせず、背後で走らせる。 */
        var H2 = global.Half2Impl;
        if (H2) {
          if (H2.initPwaInstall) { try { H2.initPwaInstall(); } catch (e) { /* 続行 */ } }
          if (H2.runAutoSync) {
            global.setTimeout(function () { H2.runAutoSync('boot'); }, 1200);
          }
        }
        return true;
      })
  ...
  var SCREEN_TITLE = {
    home: '看護師国家試験 対策',
    quiz: '', random: 'ランダムモード', tree: '単元別学習',
  ...
  var RANDOM_CARD = {
    random: { icon: 'ico-dice',   title: 'ランダムモード', meta: '単元 / 大項目',
              action: 'go-random' },
    /* アイコンは隣のカードと被らせない。
         旗は力試しモード、的は弱点ノックが使っているので、専用の2つを足した。
         横に3枚並ぶ場所で同じ絵が2つあると、どちらを押したか分からなくなる。 */
    clear : { icon: 'ico-check',  title: '全問読破',     meta: 'いじわる模試 解禁待ち',
              action: 'go-cleared' },
    evil  : { icon: 'ico-bolt',   title: 'いじわる模試', meta: '弱点だけの120問',
              action: 'go-evil' }
  };
  ...
  function randomCardState(unlearnedAtoms, evilUnlocked) {
    /* 未学習数が取れないときは、勝手に「読破」へ倒さない。
       集計が一時的に欠けただけで動線を消してしまうのが最悪の壊れ方。 */
    if (typeof unlearnedAtoms !== 'number') { return 'random'; }
    if (unlearnedAtoms > 0) { return 'random'; }
    return evilUnlocked ? 'evil' : 'clear';
  }

  function renderRandomCard(unlearnedAtoms, evilUnlocked, unlearnedQuestions) {
    var card = $('#card-random');
    var state = randomCardState(unlearnedAtoms, evilUnlocked);
  ...
  function renderAtomBody(a) {
    ...
    if (!memo) { return prepareAtomExplanation(a.explanation, a); }
  ...
  function updatePomoUi() {
    ...
    if (tg) {
      ...
      tg.textContent = p.enabled ? 'ON' : 'OFF';
  ...
  var NAG_TARGETS = { 'go-random': 1, 'go-tree': 1, 'go-exam': 1, 'go-evil': 1 };
  ...
  function routeAction(action, el) {
    switch (action) {
      ...
      case 'go-cleared':   return Half2.openAllClearedSheet();
      case 'go-evil':      return Half2.startExam('mock_weak');
      ...
      case 'go-tree':      return Half2.openTree();
   ...
   * -------------------------------------------------------------------- */
  var hooks = { afterGrade: null, afterCommit: null, onFinish: null };
  ...
  var Half2 = {
    ...
    openTree     : pending('単元別学習'),
    startByScope : pending('単元別出題'),
