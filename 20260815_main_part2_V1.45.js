...
(function (global) {
  ...
  /* ======================================================================
   * 4. キーワード検索 ＆ テーマ別 弱点分析（第12章「103概念アナライザー」）
   * ====================================================================== */
...
var QR_MATRIX = [
  ...
  /* V2.82：本番モードの配分を「いちばん本番に近い形」に寄せた。

     scheduler.js の分類にはこう書いてある。
       fresh  … 最近解いた。文面を覚えている ＝ **記憶で解ける（測定を汚す）**
       faded  … 解いたが文面を思い出せないところまで離れた ＝ **本番に最も近い**
       unseen … この問題自体が初見

     それなのに fresh を25%入れていた。**自分で「測定を汚す」と書いたものを
     4分の1入れれば、点数は実力より高く出る。** 理由はコードに書かれておらず、
     おそらく枠が埋まらないときの保険だった。

     fresh は「他が枯れたときの埋め草」に落として10%。
     faded 45% は残す。本番の国試も、受験生は過去問を解いた状態で受ける。
     「見たことはあるが忘れた」を解けるかどうかが、いちばん本番に近い測定になる。
     そのぶん未見を 30% → 45% へ。

     枠が埋まらなければ faded → fresh → unseen の順に自動で埋まるので
     （下の ORDER）、球が足りないうちは勝手に落ちる。
     **球が増えるほど本番に近づく**形になっている。

     直前モードは狙いが違う（実力測定ではなく成功体験）ので、既習55%のまま。 */
  var EXAM_MIX = {
    real : { fresh: 0.10, faded: 0.45, unseen: 0.45 },
  ...
  function runImport(text) {
    ...
    /* メモを持つ問題があるなら、取り込みの前に必ず自動退避する。
      ...
      if (rep.tag_bad) {
        lines.push('<b>⚠ 103テーマに無いタグ ' + rep.tag_bad + ' 件（' +
                   (rep.tag_bad_rows || 0) + ' 問）</b>');
        lines.push('<small>このタグは103概念の理解率にも、概念別弱点ノックにも、' +
                   '最優先克服概念にも出てきません。例：' +
      ...
      if (rep.case_orphan) {
        lines.push('<b>⚠ 連問らしいが出典が無く束ねられない ' + rep.case_orphan + ' 問</b>');
        lines.push('<small>「次の文を読み◯〜◯の問いに答えよ」と書いてありますが、' +
      ...
      if (rep.lost_bad) {
        lines.push('<b>⚠ 文字が化けている疑い ' + rep.lost_bad + ' 箇所（' +
                   (rep.lost_rows || 0) + ' 問）</b>');
        lines.push('<small>「末?循環不全」「大W骨」「法律u男女雇用機会均等法x」のように、' +
                   '文字が別のものに置き換わっています。そのまま画面に出ます。例：' +
  ...
  var HELP = {
    ...
    'drive': {
      title: 'ドライブ同期',
      body:
        '<p><b>学習の記録</b>・自分で貼った<b>図</b>・書き換えた<b>解説</b>を、' +
        '<b>あなた自身のGoogleドライブ</b>へ預けます。端末が変わっても、' +
        '同じGoogleアカウントでログインすれば戻ります。</p>' +   /* V2.32 実態合わせ */
        '<ul>' +
  ...
  var HELP_COLS = {
    ...
    col12: {
      ...
      body:
        ...
        '<li>アプリが持っている103テーマ以外の名前でも登録できます' +
  ...
  var HOME_TIPS = [
    ...
    /* --- ② 評価ボタンを押すと、次はいつ出るのか ---
       ...
       梯子：20分 → 1時間 → 1日 → 1週間 → 30日 → 90日 → 180日 */
    { id:'t05', label:'評価ボタン：難しい', title:'20分後に再出題',
      body:'いつ押しても必ず20分後に再出題されます。何度押しても20分後のままで、間隔が伸びることはありません。根拠が言えないなら、正解していても迷わずこれ。' },
    { id:'t06', label:'評価ボタン：普通',
      title:'1時間後 →翌日 →1週間に再出題',
      body:'初見なら1時間後。20分後か1時間後の段階なら翌日出題。そこから先は「普通」だけを押し続けても1週間より先へは進みません。1週間の壁を越えるには「簡単」を押す必要があります。' },
    ...
    /* t12（忘却曲線④＝マスターの話）はV2.39で削除（利用者指示：
       t08「評価ボタン：マスター」と内容が重複）。idの欠番はそのまま＝
       文言の自分直し（text_overrides）のキーを揺らさない。 */
    ...
    { id:'t36', label:'マイ★お気に入りノート', title:'★は5段階に育ちました',
      body:'★はタップのたびに ★1→★2→★3→★4→★5→解除 と一周します。どの段階を何に使うかは自由（例：★1=疑問、★2=再学習）。段階ごとに色が変わるので、ノートを開けばひと目で見分けられます。' },
    { id:'t37', label:'マイ★お気に入りノート', title:'段階で絞る・段階に名前を付ける',
      body:'★ノートでは段階（★2だけ等）で絞り込めます。間違いノート印刷も「★2だけ印刷」ができます。段階の名前は★ノートの中から付けられます。' },
  ...
  var TIPS = {
    ...
    home_tip: { step:'下の一言',     sel:'#home-tip',
                text:'ここに使い方と考え方を1つずつ出しています。' +
                     '［次へ］で送り、［前へ］で戻れます。' },   /* V2.39 件数の固定記載をやめた */
