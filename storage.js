...
(function (global) {
  ...
  /* V2.52：出題基準マスタが使っている全角記号は、半角へ変えない。
     マスタ465キーに実在するのは （ ） ＜ ＞ ／ ～ の6文字。
     ここを半角化すると、取り込んだ問題の保存値だけがマスタと食い違い、
     同じ単元が3階層ツリーで二股に割れる（実測：配布176問のうち21問が
     「在宅看護論/地域・在宅看護論」という別単元として生えた）。
     アプリが自分で半角化した値を、自分の全角マスタと突き合わせて
     「出題基準に無い分類」と警告していた。
     狙いは「１. 健康の定義」→「1. 健康の定義」の吸収であって、
     分類名の記号を書き換えることではない。 */
  var TAX_KEEP_WIDE = '（）＜＞／～';
  function taxKey(s) {
  ...
  function normalizeTaxKey(v) {
    return String(v == null ? '' : v)
      ...
      .replace(/[＜＞]/g, function (m) { return m === '＜' ? '<' : '>'; })
      /* V2.51：スラッシュの全角・半角を揃える。
         「在宅看護論／地域・在宅看護論」（マスタ・全角）と
         「在宅看護論/地域・在宅看護論」（取り込み・半角）は
         人の目で見分けられない。実測で配布JSON 176問のうち
         21問がこれだけの理由で「出題基準に無い分類」と数えられ、
         3階層ツリーが単元から二股に割れていた。
         中黒（・）は意味のある区切りなので触らない。 */
      .replace(/／/g, '/')
      .replace(/[\s\u3000]+/g, ' ')
      /* V2.75：先頭の「番号＋ピリオド」の直後の空白は意味を持たないので寄せる。
         実測：出題基準マスタは「3. 女性のライフサイクル各期における看護」で、
         取り込んだ側は「3.女性のライフサイクル各期における看護」。
         **スペース1つの違いだけ**で「出題基準に無い分類」と数えられ、
         第115回午後問64 が1問だけ落ちていた。

         空白を全部消さないのは、分類名の中の空白には意味があるため
         （「A. 人体の基本的な構造 and 正常な機能」のような語の区切り）。
         寄せるのは**先頭の番号の直後だけ**にする。 */
      .replace(/^([0-9A-Za-z]{1,3})\.\s+/, '$1.')
      .trim();
  }
