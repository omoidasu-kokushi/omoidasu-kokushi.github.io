...
(function (global) {
  ...
  function openRandomSelect() {
    return Promise.all([S.buildTree(), S.loadMeta()]).then(function (r) {
      ...
      setText('#unit-hero-title', u.label + ' 全体ランダム');
      setText('#unit-hero-sub', '全 ' + u.count + ' 問から出題');
      $('#unit-hero').dataset.unit = u.key;

      /* 中項目階層を排し、単元 → 大項目 の2段階だけを見せる */
      setHtml('#major-list', u.children.map(function (mj) {
        return '<button type="button" class="major-card" data-major="' + esc(mj.key) + '">' +
               '<span class="major-name">' + esc(mj.label) + ' ランダム</span>' +
               '<span class="major-count">' + mj.count + '問</span>' +
               (mj.unlearned ? '<span class="badge-line">' + (mj.unlearned > 99 ? '99+' : mj.unlearned) + '</span>' : '') +
               '</button>';
      }).join(''));
  ...
  function startRandom(scope, count) {
    ...
    return M.startSession({
      mode: 'random',
      count: st.random.count,
      scope: scope || null,
      newOnly: true,
      shuffle: true
    }).then(function (sess) {
      if (!sess) { openModal('#modal-no-new'); }
      return sess;
    });
  }
  ...
  function openTree() {
    return S.buildTree().then(function (tree) {
      if (!tree.length) { toast('先に設定から問題データを取り込んでください'); return; }
      setHtml('#tree-root', tree.map(function (u) {
        return node(u, 'unit', u.children.map(function (mj) {
          return node(mj, 'major', mj.children.map(function (md) {
            return '<button type="button" class="tree-leaf" data-scope="medium" data-value="' + esc(md.key) + '">' +
                   '<span class="tree-label">' + esc(md.label) + '</span>' +
                   badge(md.unlearned) + '<span class="tool-arrow"></span></button>';
          }).join(''));
        }).join(''));
      }).join(''));
      return M.go('tree');
    }).then(function (r) { global.setTimeout(function () { tip('tree'); }, 500); return r; });

    function badge(n) {
      return n ? '<span class="badge-line">' + (n > 99 ? '99+' : n) + '</span>' : '';
    }
    function node(item, scope, childrenHtml) {
      return '<div class="tree-node">' +
             '<button type="button" class="tree-head" data-toggle="1">' +
             '<span class="tree-caret"></span>' +
             '<span class="tree-label">' + esc(item.label) + '</span>' +
             badge(item.unlearned) +
             '<span class="major-count">' + item.count + '問</span>' +
             '</button>' +
             '<div class="tree-children">' + childrenHtml +
             /* 単元を開くと、大項目の「まとめて出題」と単元の「まとめて出題」が
                同じ文言で縦に並び、どちらがどの範囲か分からなくなっていた。
                範囲の名前を必ず文面に入れる。 */
             '<button type="button" class="tree-leaf is-scope" data-scope="' + scope +
             '" data-value="' + esc(item.key) + '">' +
             '<span class="tree-label">▶ 「' + esc(item.label) + '」をまとめて出題</span>' +
    ...
    return S.getUserAudio().then(function (rec) {
      ...
      if (rec && rec.blob) {
        setText('#alarm-file-note',
          '入っています（' + Math.max(1, Math.round(rec.bytes / 1024)) + 'KB）。' +
          '「アラーム音」で「自分の音」を選ぶと鳴ります。');
  ...
  function fmtExamNote(meta) {
    ...
    if (rest < 0) {
      return 'この日は過ぎています。間隔の上限は外れ、通常（最長180日）に戻っています。';
    }
  ...
  function refreshExamNote() {
    return S.loadMeta().then(function (m) {
      M.state.meta = m;
      var inp = $('#set-exam-date');
      if (inp) { inp.value = m.exam_date || ''; }
  ...
  function setExamDate(value) {
    ...
    if (v && !K.parseExamDate(v, 4)) {
      toast('日付として読めませんでした', 3000);
      return Promise.resolve(null);
    }
    return S.setMeta('exam_date', v).then(function () {
      return refreshExamNote();
    }).then(function (m) {
      toast(v ? '試験日を設定しました' : '試験日を消しました', 2400);
      return m;
    });
  }
  ...
  function refreshDrive() {
    ...
    return Promise.all([D.getClientId(), D.lastSync(), D.hasConsent()]).then(function (r) {
      ...
      var login = $('#btn-drive-login'), logout = $('#btn-drive-logout'), sync = $('#btn-drive-sync');
      if (login)  { login.hidden = signedIn; login.disabled = !id; }
      if (logout) { logout.hidden = !signedIn; }
      if (sync)   { sync.disabled = !signedIn; }
      return { id: id, signedIn: signedIn, last: last, consent: consent,
               builtIn: builtIn, own: own };
      });
    });
  }
  ...
  function driveSync() {
    ...
    return D.syncNow(function (m) { setText('#drive-note', m); }).then(function (rep) {
  ...
  var impl = {
    ...
    openTree: openTree,                  startByScope: startByScope,
  ...
  function bind() {
    ...
    on($('#unit-hero'), 'click', function (ev) {
      startRandom({ field: 'unit', value: ev.currentTarget.dataset.unit }, st.random.count);
    });
    on($('#major-list'), 'click', function (ev) {
      var c = ev.target.closest('.major-card');
      if (c) { startRandom({ field: 'major', value: c.getAttribute('data-major') }, st.random.count); }
    });
    ...
    /* --- 3階層ツリー --- */
    on($('#tree-root'), 'click', function (ev) {
      var leaf = ev.target.closest('.tree-leaf');
      if (leaf) { startByScope(leaf.getAttribute('data-scope'), leaf.getAttribute('data-value')); return; }
      var head = ev.target.closest('.tree-head');
    ...
    on($('#warn-study'), 'click', function () { closeModals(); openTree(); });
    ...
    on($('#set-exam-date'), 'change', function (ev) { setExamDate(ev.target.value); });
    on($('#btn-exam-clear'), 'click', function () {
      var i = $('#set-exam-date'); if (i) { i.value = ''; }
      setExamDate('');
    });
