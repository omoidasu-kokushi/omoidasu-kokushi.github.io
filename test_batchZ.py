...
with sync_playwright() as p:
    ...
    # ---------- 同期ボタン（V1.44でヘッダーから撤去） ----------
    # ボタンが多すぎて 390px 幅で溢れたため、同期は設定の中だけにした。
    # 未同期の件数は設定ボタンのバッジに残す（どこにも出ないと溜まる）。
    r = pg.evaluate("""async () => {
      const H = window.Half2Impl;
      await window.Storage.clearDirty();
      await window.Storage.bumpDirty(2);
      await H.refreshHdrSync();
      const b = document.getElementById('hdr-sync-badge');
      return { inHeader: !!document.getElementById('btn-hdr-sync'),
               badgeUnderSettings: !!document.querySelector('#btn-settings #hdr-sync-badge'),
               hidden: b.hidden, text: b.textContent };
    }""")
    ok("同期ボタンはヘッダーに無い", r["inHeader"] is False, json.dumps(r))
    ok("未同期バッジは設定ボタンに付いている", r["badgeUnderSettings"] is True, json.dumps(r))
    ok("未同期があればバッジが出る",
       r["hidden"] is False and r["text"] == "2", json.dumps(r))
