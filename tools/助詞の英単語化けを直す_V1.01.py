# -*- coding: utf-8 -*-
"""助詞の英単語化けを直す_V1.01.py

V1.01：**2語つづきの化け**を足す。V1.00 は1語ずつしか見ていなかった。

    誤 食生活 of the 教育                      正 食生活の教育
    誤 #看護管理·医療提供 of the 仕組み          正 #看護管理・医療提供の仕組み
    誤 安全管理体制整備、医療安全文化 of the 醸成  正 …医療安全文化の醸成

  実測：`of the` が8件残っていた。V1.00 の `of` 規則は
  「日本語 of 日本語」を見るので、あいだに `the` が挟まると当たらない。

V1.01：中黒の化けも直す（タグと分類のときだけ）。

    `・`(U+30FB) が `·`(U+00B7) や `･`(U+FF65) に化ける。見た目がほぼ同じ。
    実測：#医療安全·アクシデント防止 が、マスタの #医療安全・アクシデント防止 と
    一字違いになり「マスタに無いタグ」と数えられていた（3種13件）。
    本文には掛けない。中黒は本文では別の意味で使われうる。
...
FIX = [
    # V1.01：2語つづき。1語の規則より先に置く（先に of だけ食われないように）
    (re.compile(r'(' + J + r')\s?\bof\s+the\b\s?(' + J + r')'), r'\1の\2'),
    (re.compile(r'(' + J + r')\s?\band\s+the\b\s?(' + J + r')'), r'\1と\2'),
...
# V1.01：中黒の化け。タグ・分類のように「マスタと一字一句で比べるもの」だけに掛ける。
_NAKAGURO = {'\u00b7': '\u30fb', '\uff65': '\u30fb', '\u2027': '\u30fb'}


def fix_nakaguro(s):
    """中黒の化けを直す。**本文には使わない**（本文の中黒は別の意味を持ちうる）。

    タグ・sub_item・unit/major/medium/target のように、
    マスタと一字一句で突き合わせる欄にだけ掛ける。"""
    if not s:
        return s, 0
    n = 0
    for bad, good in _NAKAGURO.items():
        k = s.count(bad)
        if k:
