...
def judge_splittable(q):
    """(決まったか, 値, 理由) を返す。決まらなければ (False, None, 理由)。"""
    stem = str(q.get("stem") or "")
    if q.get("question_type") != "single":
        return True, False, "複数選択・計算問題"
    for w in SPLIT_NG_WORDS:
        if w in stem:
            return True, False, "相対判断の設問（%s）" % w
    if q.get("case_key") or "次の文を読み" in stem:
        return False, None, "連問（事例文つき）"
    if q.get("image_url"):
        return False, None, "図つき"
    if len(stem) > 120:
        return False, None, "問題文が長い（%d字）" % len(stem)
    return True, True, "切り出してよい"


def fill_splittable(q, stat, pending):
    """空のときだけ埋める。**既に書いてある値は触らない**（§鉄則）。"""
    if q.get("is_splittable") is not None:
        return
    decided, val, why = judge_splittable(q)
    if decided:
        q["is_splittable"] = val
        stat["埋めた一問一答の可否"] = stat.get("埋めた一問一答の可否", 0) + 1
    else:
        pending.append((q.get("source"), why, (q.get("stem") or "")[:60]))
...
def main():
    ...
    if pending:
        ...
        pf = os.path.join(outd, "一問一答の判断待ち_%s.tsv"
        ...
        print("一問一答の可否が規則で決まらない %d問 → %s"
              % (len(set(s0 for s0, _, _ in pending)), os.path.basename(pf)))
        for k, v in cc2.most_common():
            print("    %-18s %4d問" % (k, v))
