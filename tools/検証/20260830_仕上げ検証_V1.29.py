# -*- coding: utf-8 -*-
"""過去問仕上げの出力を機械で検証する（V1.29）。

V1.29（2026-09-09）：重大度の呼び名を「警告」→「確認」に変えた。
判定は1つも変えていない。「警告」だと壊れているように読まれるが、実際は
「配布は止めない。人が見てもいい」という印でしかない。呼び名が強すぎると、
本当に見るべきものが埋もれる。

V1.28（2026-09-09）：作り直しの突き合わせに使う鍵を、生の出典名から
**空白を詰めた形**に変えた。出典名の空白が1つと2つで別の問題として数えられ、
同じ問題が2回配布されていた（実測16問）。

V1.27（2026-09-09）：偽陽性を2つ潰した。どちらも**検査を緩めたのではなく、
見るものを正しくした**。
  ・チェック6：入力の化けを直してから数値を拾う（第114回午前問100「97」）
  ・チェック9：かぎかっこの中（問題文からの引用）はですます調判定から外す
    （第112回午後問109「薬は飲み忘れたんです」）

V1.26（2026-09-09）：文字化けを直す道具が見つからないときに、黙って
検査の一部を無効化していたのをやめた。探す場所を5か所に増やし、
それでも無ければ理由を書いて止める。実測で NG が 33問→98問 に化けていた。
...
# V1.26（2026-09-09）：**見つからなかったときに黙らない。**
#
# 何が起きていたか：探す場所が ~/omo/tools だけで、見つからないと except で
# _garble = _kakko = None にして、そのまま何も言わずに走っていた。
# チェック2は入力と出力を一字一句比べる検査で、**入力（PDFから機械抽出した
# 元データ）そのものに化けが入っている**ため、両側に同じ直しを掛けてから
# 比べる必要がある。道具が無いと入力側だけ化けたままになり、正しく直した
# 出力が全部「書き換えられた」と判定される。
#
# 実測（2026-09-09・同じフォルダ／同じコマンド）
#   道具が見つかる環境   チェック2 NG  16件 ／ 全体 NG 33問（2.7%）
#   見つからない環境     チェック2 NG 103件 ／ 全体 NG 98問（8.0%）
#   例：第111回午前問11「右大W骨」（入力の化け）↔「右大腿骨」（直した出力）
#
# 数字が3倍になるのに画面には何も出ない。これがいちばん悪い。
# 探す場所を増やし、それでも見つからなければ**大きく知らせて止める**。
# どうしても先へ進めたいときだけ --化け直し無し を付ける。
_TOOL_DIRS = [
    os.path.dirname(os.path.abspath(__file__)),          # この台本と同じ場所
    P('tools'),                                           # 分類フォルダの下
    os.path.expanduser('~/omo/tools'),                    # 手元のリポジトリ
    os.path.join(os.path.expanduser('~'), 'Desktop', 'omo', 'tools'),
    os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 '..', '..', '..', 'omo', 'tools')),
]
_GARBLE_NAMES = ['助詞の英単語化けを直す_V1.01.py', '助詞の英単語化けを直す_V1.00.py']
_KAKKO_NAMES = ['括弧と欠字の化けを直す_V1.01.py', '括弧と欠字の化けを直す_V1.00.py']


def _find_tool(names):
    """置いてある場所を順に探し、見つかった実体（path）を返す。無ければ None。"""
    for d in _TOOL_DIRS:
        for n in names:
            f = os.path.join(d, n)
            if os.path.exists(f):
...
_garble = _kakko = None
_garble_path = _find_tool(_GARBLE_NAMES)
_kakko_path = _find_tool(_KAKKO_NAMES)
_tool_err = []
for _p, _nm, _which in ((_garble_path, '_g', 'garble'), (_kakko_path, '_k', 'kakko')):
    if _p is None:
        _tool_err.append('見つからない：' + (_GARBLE_NAMES[0] if _which == 'garble'
                                             else _KAKKO_NAMES[0]))
        continue
    try:
        _m = _load(_p, _nm)
        ...
        _tool_err.append('読めない：%s（%s）' % (os.path.basename(_p), str(_e)[:60]))

if _tool_err and '--化け直し無し' not in sys.argv:
    sys.stderr.write(
        '\n' + '=' * 64 + '\n'
        '文字化けを直す道具が読み込めません。このまま検証すると、\n'
        'チェック2（入力と出力が一字一句同じか）が**大量に誤検出**します。\n'
        '入力データそのものに化けが入っているため、両側に同じ直しを\n'
        '掛けないと比べられません。実測では NG が 33問 → 98問 に増えました。\n\n'
        + '\n'.join('  ・' + x for x in _tool_err) + '\n\n'
        '探した場所：\n' + '\n'.join('  ' + d for d in _TOOL_DIRS) + '\n\n'
        'どちらかをしてください。\n'
        '  ① 上のどれかの場所に、omo リポジトリの tools/ にある2本を置く\n'
        '  ② 誤検出を承知で進めるなら --化け直し無し を付けて実行する\n'
        + '=' * 64 + '\n')
    sys.exit(2)
if _tool_err:
    print('※ --化け直し無し で実行しています。チェック2は誤検出が大量に出ます')
...
def defix(s):
    """化けだけ直して返す（空白は詰めない）。V1.27。

    normc() は比較用に空白まで落とすので、数値を拾う用途には使えない
    （「1,000 円」の桁区切りは残したいが、空白の有無で別物になると困る）。
    ここでは化け直しだけを掛ける。"""
    t = s or ''
...
def check(q, ref, ans=None):
    ...
    if subs and nsub not in nsubs and not cut:
        if smaster and nsub not in smaster:
            ...
            ng.append((-4, 'sub_item「%s」は出題基準にあるが、この問題の候補外（確認）'
                       % q.get('sub_item')))
    ...
    for a in qa:
        for t in (a.get('tags') or []):
            ...
            if master and nt not in master:
                ...
                ng.append((-4, '肢%s のタグ「%s」はマスタにあるが、この問題の候補外（確認）'
                           % (a.get('original_num'), t)))
    ...
    # 6 数値の裏づけ
    # V1.27：入力の stem にも化け直しを掛けてから数値を拾う。
    #
    # チェック2 は V1.11 から「両側に同じ直しを掛けてから比べる」ようにしてあったが、
    # チェック6（数値の裏づけ）は生の入力から拾っていた。
    # 実測：第114回午前問100 の入力は
    #     経皮的動脈血酸素飽和度?SpO2C97 %      （山かっこが ? と C に化けている）
    # で、numbers() は「英字の直後の数字は語の一部」という規則があるため
    # C97 の 97 を拾えず、7 しか取れない。出力は正しく
    #     経皮的動脈血酸素飽和度〈SpO2〉97 %
    # と直っているので「97 に裏づけが無い」と誤って言っていた。
    # 直したあとの入力から拾えば、そのまま裏づけになる。
    ok = set()
    for a in qa: ok |= numbers(a.get('text')) | numbers(defix(a.get('text')))
    ok |= numbers(o.get('stem')) | numbers(defix(o.get('stem')))
    ...
    narrow = set()
    for a in qa: narrow |= numbers(a.get('text')) | numbers(defix(a.get('text')))
    narrow |= numbers(o.get('stem')) | numbers(defix(o.get('stem'))) | numbers(q.get('evidence'))
        ...
        if r is None: ng.append((7, 'evidence の標準値表id「%s」が表に無い' % i))
            ...
            if r['確度'] == 'X':
                ...
                if wrote:
                    ...
                    ng.append((-7, '%s は確度Xの行。数値は書いていないので通す（確認）' % i))
    ...
    for where, t in texts:
        # V1.27：**かぎかっこの中は見ない**。
        # 実測：第112回午後問109 の全体解説は「である」調だが、問題文からの引用
        #     A さんは訪問看護師に対して「薬は飲み忘れたんです。心配かけて…」
        # を含むため、ですます調と判定されていた。引用は原文のまま載せるのが
        # 正しく、直させると出典を書き換えることになる。
        body = re.sub(r'[「｢][^」｣]{0,200}[」｣]', '', t or '')
        if re.search(r'(です|ます)[。、]', body): ng.append((9, '%s がですます調' % where))
    ov = norm(q.get('overall_explanation'))
    for a in qa:
        e = norm(a.get('explanation'))
        for i in range(0, max(0, len(e) - 35)):
            if e[i:i + 35] and e[i:i + 35] in ov:
                ng.append((-9, '肢%s の解説が全体解説と35字以上重複（確認）' % a.get('original_num'))); break
    ...
    for a in qa:
        ...
        if not wk:
            ng.append((-10, '肢%s の論点 %s は裏づけの取れない数値だけでできている。照合を免除（確認）'
...
def main():
    ...
    for f in files:
        ...
        if keep_all:
            ...
            for q in got:
                # V1.28：**空白を詰めた形を鍵にする。**
                # 「第113回  午後問117」（空白2つ）と「第113回 午後問117」が
                # 別の問題として数えられ、同じ問題が2回配布されていた（実測16問）。
                # 掃除 V1.07 が出典名を入力バッチの表記へそろえるので根は絶たれたが、
                # 掃除を通さずにここへ来る経路が残るため、鍵の側でも1つに寄せる。
                src = tidy_src(q.get('source') or ('?' + os.path.basename(f)))
                if src in newest:
                    dup_dropped += 1
                    if mt <= newest[src][0]:
                        continue
    ...
    for q in qs:
        for n, msg in check(q, ref, ANS.get(q.get('source'))):
            # V1.29：重大度の呼び名を「警告」→「確認」に変えた。判定は1つも変えていない。
            # 「警告」だと『何かが壊れている』と読まれるが、実際は
            # 「配布は止めない。人が見てもいい」という印でしかない。
            # 実測652件のうち437件は欄が空なだけ、140件は言い回しの重複だった。
            # 呼び名が強すぎると、本当に見るべきものが埋もれる。
            log.append((q.get('source', '?'), abs(n), ('確認' if n < 0 else 'NG'), msg))
            if n < 0: warn += 1
    ...
    print('検証 %d問 ／ NG（配布しない） %d問（%.1f%%） ／ 指摘 %d件（うち確認 %d件）' %
