# -*- coding: utf-8 -*-
"""頼んだ問題と違うものが返ってきていないか調べる V1.01（2026-09-10）

【V1.01】捨て駒（M0000）は「捨て駒」と表示して、違う／未処理に数えない。
  体験用のバッチを作る道具 V1.01 が、先頭に M0001 と同じ中身の M0000 を置くようになった。
  その返りは前回の最後の回答なので、突き合わせれば必ず「頼んだ問題と違う」になる。
  想定どおりのずれを NG に数えると、本当のずれ（1本ずれ）が埋もれる。
  V1.00 の判定は1つも変えていない（レーン対応は V1.00 の中で 2026-09-10 に足したもの）。
...
【使い方】
  python3 注文と違う出力を見つける_V1.01.py
  python3 注文と違う出力を見つける_V1.01.py --base <分類フォルダ>
  python3 注文と違う出力を見つける_V1.01.py --lane free   # 体験用の模試だけ見る
  python3 注文と違う出力を見つける_V1.01.py --selftest
"""
import glob
import io
import json
...
def is_dummy(name):
    """捨て駒か。番号 0000（M0000）だけ。過去問レーンの捨て駒（v7再生成 V1.06）は
    本命と同じ番号帯に置かれるので、ここでは見分けない。"""
    return name[1:] == '0000'
...
def check(base, bdir='batches_finish_v7', rdir='res_finish', pat='I*.txt'):
    ...
    for b in sorted(glob.glob(os.path.join(bdir, pat))):
        ...
        want = asked(b)
        if is_dummy(name):
            # 【V1.01】捨て駒。返りは前回の答えなので突き合わせない
            rows.append((name, sorted(want), None, '捨て駒'))
            continue
...
def main():
    ...
    for k in lanes:
        ...
        bad = [r for r in rows if r[3] not in ('OK', '捨て駒')]
        for name, want, got, why in rows:
            mark = 'ok ' if why == 'OK' else ('-- ' if why == '捨て駒' else 'NG ')
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 【V1.01】捨て駒 I0000（中身は I0001 と同じ・返りは前回の答え）
        io.open(os.path.join(b, 'I0000.txt'), 'w', encoding='utf-8').write(
            'なにか説明\n{"questions":[{"source":"第112回 午前問64"}]}')
        io.open(os.path.join(r, 'I0000.json'), 'w', encoding='utf-8').write(
            json.dumps({'questions': [{'source': '第110回 午前問99'}]}, ensure_ascii=False))
        rows = {x[0]: x for x in check(td)}
        ok('捨て駒は「捨て駒」と出し、違うに数えない', rows['I0000'][3] == '捨て駒', rows['I0000'])
