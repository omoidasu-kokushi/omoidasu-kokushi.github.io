#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""仕上げ結果（res_finish/*.json）を、アプリ取り込み用の1本のJSONへ結合する道具（V1.01）。
...
_FIELDS = ["stem", "overall_explanation", "unit", "major", "medium", "target", "evidence"]
...
def main():
    ...
    if rep['excluded_deleted']:
        ...
        for s in rep['excluded_deleted']:
            print('  ', s)
    print('英単語化け：直した %d件' % rep['degarbled'].get('fixed', 0))
    if rep['watch']:
