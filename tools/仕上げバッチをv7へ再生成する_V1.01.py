# -*- coding: utf-8 -*-
"""仕上げバッチの再生成（v7）。V1.01 — 2026-09-09
...
def emit(base, tmap, skip, harvested):
    ...
    counters = {'new': 0, 'hisshu': 1000, 'ng': 5000}
    ...
    for unit, pairs in groups:
        for kind in ('new', 'hisshu', 'ng'):
            ...
            if sub:
                write(unit, sub, kind)
    n = counters['new'] + (counters['hisshu'] - 1000) + (counters['ng'] - 5000)
...
def selftest():
    ...
    try:
        ...
        # S1 は合格なのでどちらにも入らない。
        newf = io.open(os.path.join(tmp, 'batches_finish_v7', 'I0001.txt'), encoding='utf-8').read()
        ngf  = io.open(os.path.join(tmp, 'batches_finish_v7', 'I5001.txt'), encoding='utf-8').read()
        ...
        mf = io.open(os.path.join(tmp, 'batches_finish_v7', 'I1001.txt'), encoding='utf-8').read()
        assert 'M1' in mf, '必修のNGは I1001〜'
        ...
        print('selftest OK (10観点：V1.01の7つ＋番号の振り分け3つ)')
    finally:
        shutil.rmtree(tmp)
...
if __name__ == '__main__':
    ...
    print('  I0001〜 …… 出力が1度も返ってこなかった問題（先に流れます）')
    print('  I1001〜 …… 必修のNG（無料版に全部入れるので優先）')
    print('  I5001〜 …… 必修以外のNG')
