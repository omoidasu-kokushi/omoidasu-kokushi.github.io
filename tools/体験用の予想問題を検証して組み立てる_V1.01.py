# -*- coding: utf-8 -*-
"""体験用の予想問題（無料版の模試）を検証して、取り込み用の1本にまとめる V1.01

【V1.01】（2026-09-10）捨て駒 M0000 を見ない。注文より古い出力は NG にする。

  1. 捨て駒 M0000
     バッチを作る道具 V1.01 は、先頭に M0001 と同じ中身の M0000 を置く
     （PADが各実行の1本目に前回の最後の回答を保存するため）。返ってきた M0000.json は
     前回の答えなので、注文と突き合わせれば必ず NG になる。読まずに、あることだけ報告する。

  2. 注文より古い出力
     案A（作り直し）で batches_free を再生成したあと、res_free に前回の85問が
     残ったまま PAD を流すと、途中で止めたときに新旧が混ざる。出力の更新時刻が
     注文の更新時刻より古ければ「前回の出力が残っている」として NG にする。
     （V1.00 は「まだ返ってきていない」しか見ていなかった）

  V1.00 の判定は1つも消していない。selftest は 13 → 17 項目。

V1.00：2026-09-10 作成。selftest 13/13 通過、85問の本番検証も動いた（2026-09-10）。
...
【使い方】
  python 体験用の予想問題を検証して組み立てる_V1.01.py
  python 体験用の予想問題を検証して組み立てる_V1.01.py --base <分類フォルダ>
  python 体験用の予想問題を検証して組み立てる_V1.01.py --selftest
"""
import collections
import datetime
import glob
...
DUMMY = "M0000"      # 捨て駒（バッチを作る道具 V1.01）。注文にも出力にも数えない
...
def load_orders(base):
    """バッチが頼んだ内容を M番号 → [注文] で返す。捨て駒 M0000 は読まない。"""
    out = {}
...
def stale(base, name):
    """【V1.01】出力が注文より古いか。古ければ前回の出力が残っている。

    PADは注文を読んでから書くので、正しい出力は必ず注文より新しい。
    注文を作り直したあとに残った前回の出力だけが、注文より古くなる。"""
    b = os.path.join(base, "batches_free", name + ".txt")
    r = os.path.join(base, "res_free", name + ".json")
    try:
        return os.path.getmtime(r) < os.path.getmtime(b)
    except OSError:
        return False
...
def verify(base):
    ...
    for name in sorted(orders):
        ...
        if got is None:
            ...
            continue
        # 【V1.01】注文より古い出力＝前回の出力が残っている。新旧が混ざるので NG
        if stale(base, name):
            ng.append((name, "前回の出力が残っている（注文 %s.txt より古い）。"
                             "res_free を空にしてから PAD を流し直す" % name))
            continue
        if len(got) != len(want):
...
def report(base, ng, warn, rows):
    ...
    if os.path.exists(os.path.join(base, "res_free", DUMMY + ".json")):
        print("  捨て駒 %s の返りは見ていない（前回の最後の回答が入る前提）" % DUMMY)
    if not rows:
        return
    for v in ("free_a", "free_b"):
        sub = [q for _, q in rows if q.get("variant") == v]
...
def selftest():
    ...
    # 【V1.01】捨て駒 M0000 と、注文より古い出力
    import tempfile
    import time
    tmp = tempfile.mkdtemp()
    try:
        bd = os.path.join(tmp, "batches_free")
        rd = os.path.join(tmp, "res_free")
        os.makedirs(bd)
        os.makedirs(rd)
        order = {"unit": "必修", "major": "1. あ", "medium": "A. い", "rank": "A",
                 "variant": "free_a", "pool": "mock", "question_type": "single",
                 "select_count": 1, "atom_count": 2, "sub_candidates": [],
                 "tag_candidates": []}
        q = {"unit": "必修", "major": "1. あ", "medium": "A. い", "rank": "A",
             "variant": "free_a", "pool": "mock", "question_type": "single",
             "select_count": 1, "source": None, "origin_key": None,
             "stem": "検査用の問題はどれか。", "overall_explanation": "全体の解説である。",
             "evidence": None,
             "atoms": [{"original_num": 1, "is_correct": True, "text": "短い言葉",
                        "statement": "短い言葉が正しい。", "explanation": "正しい。", "tags": []},
                       {"original_num": 2, "is_correct": False, "text": "別の言葉",
                        "statement": "別の言葉は違う。", "explanation": "違う。", "tags": []}]}
        body = "見出し\n" + json.dumps({"orders": [order]}, ensure_ascii=False)
        for n in ("M0000", "M0001"):
            io.open(os.path.join(bd, n + ".txt"), "w", encoding="utf-8").write(body)
        # 捨て駒の返りは「前回の最後の回答」＝注文と違う中身
        io.open(os.path.join(rd, "M0000.json"), "w", encoding="utf-8").write(
            json.dumps({"questions": [dict(q, unit="成人看護学", pool="main")]}, ensure_ascii=False))
        io.open(os.path.join(rd, "M0001.json"), "w", encoding="utf-8").write(
            json.dumps({"questions": [q]}, ensure_ascii=False))
        now = time.time()
        for n in ("M0000", "M0001"):
            os.utime(os.path.join(bd, n + ".txt"), (now - 100, now - 100))
            os.utime(os.path.join(rd, n + ".json"), (now, now))
        ng1, _w, rows1 = verify(tmp)
        ok("捨て駒 M0000 は注文にも出力にも数えない",
           "M0000" not in load_orders(tmp) and "M0000" not in load_results(tmp)[0])
        ok("捨て駒の返りが注文と違っても NG にしない",
           not [x for x in ng1 if x[0].startswith("M0000")], ng1[:3])
        ok("本命 M0001 は普通に検証される（NG なし・1問）",
           not [x for x in ng1 if x[0].startswith("M0001")] and len(rows1) == 1, ng1[:3])
        # 注文を作り直したあと、前回の出力が残っている
        os.utime(os.path.join(bd, "M0001.txt"), (now + 100, now + 100))
        ng2, _w, rows2 = verify(tmp)
        ok("注文より古い出力は「前回の出力が残っている」で NG",
           any("前回の出力が残っている" in b for a, b in ng2 if a == "M0001") and not rows2,
           ng2[:3])
    finally:
        import shutil
