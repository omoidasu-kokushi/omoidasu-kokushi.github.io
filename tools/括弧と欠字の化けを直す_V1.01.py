# -*- coding: utf-8 -*-
"""括弧と欠字の化けを直す V1.01（2026-09-07）

V1.01：化けの組が**3組**あることが分かった。V1.00 の2組では取りこぼした。

    〈 → ?    〉 → C     （V1.00 で発見）
    〈 → u    〉 → x     （V2.64 で既知）
    〈 → V    〉 → Z     （V1.01・今回の実測）
        連続性副雑音は笛のような音V笛音Zである → 音〈笛音〉
        自我同一性VアイデンティティZの確立     → 自我同一性〈アイデンティティ〉

  もう1つ取りこぼしがあった。開き括弧の**手前の文字**を
  「日本語か半角英数字」に限っていたので、
    Barréuギラン・バレーx症候群
  の é に当たらなかった。アクセント付きのラテン文字も手前として認める。
...
# --- 対になっているときだけ直す（開き→閉じ） ---
# 手前に来てよい文字。V1.01：アクセント付きラテン文字（é など）を足した。
LEAD = "[" + JP + r"A-Za-zÀ-ÖØ-öø-ÿ0-9]"

# 化けの3組。開きと閉じが**対になっているときだけ**直す。
# 中に同じ組の文字を挟まない・中身は1〜24文字。
PAIRS = [
    (re.compile(r"(" + LEAD + r")\?([^?C\n]{1,24})C"), r"\1〈\2〉"),
    (re.compile(r"(" + LEAD + r")u([^ux\n]{1,24})x"), r"\1〈\2〉"),
    (re.compile(r"(" + LEAD + r")V([^VZ\n]{1,24})Z"), r"\1〈\2〉"),
]
...
# --- 直さず数えるだけ ---
WATCH = re.compile(r"[" + JP + r"]\s?[?WuxCVZ]\s?[" + JP + r"]")
...
def selftest():
    cases = [
        ...
        # V1.01：3組目と、アクセント付きの手前文字
        ("笛のような音V笛音Zである", "笛のような音〈笛音〉である"),
        ("自我同一性VアイデンティティZの確立", "自我同一性〈アイデンティティ〉の確立"),
        ("Barréuギラン・バレーx症候群", "Barré〈ギラン・バレー〉症候群"),
        # V や Z が本物で出るときは触らない
        ("V字型の切開", "V字型の切開"),
        ("Z形成術を行う", "Z形成術を行う"),
        ("ビタミンB12の欠乏", "ビタミンB12の欠乏"),
    ]
    bad = 0
