# -*- coding: utf-8 -*-
"""仕上げバッチの再生成（v7）。V1.06 — 2026-09-09

【V1.06】**捨て駒を1本目に置く。**

  PADは各実行の1本目に、前回の最後の回答を保存する（実測 2026-09-09）。
    27本のバッチの最後は「第114回 午後問116」
      → 次の2本のバッチの1本目 I10631 は「第112回 午前問64」を頼んだのに
        中身は「第114回 午後問116」だった
    その2本の最後は「第113回 午後問117」
      → 次の1本のバッチ I12232 も同じく1本ずれた

  生成が終わる前に画面をコピーしているため。2本目以降は正しく返る。
  **1本だけのバッチは必ず失敗する。**しかも画面には何も出ないので、
  回したつもりで終わる。第112回 午前問64 は3回頼んで3回とも作られなかった。

  そこで、バッチが少ないときは**同じ内容をもう1本、先頭に置く**。
  1本目は捨てられる前提で、2本目が本命。
  （PAD側で「回答が出るまで待つ」を直せるなら、そちらが本筋。
    この道具の捨て駒は、直すまでのしのぎ。）

  --捨て駒なし を付けると、この振る舞いを止められる。

【V1.05】source を**空白を詰めた形**で突き合わせる。

  実測：第113回 午後問117 は、作り直しのどこかで source が
  「第113回  午後問117」（全角スペース2つ）に化けた。res_finish には
  化けていない古い版と、化けた新しい版が**別の source として両方ある**。

  仕上げ検証は source を正規化して見る（tidy_src）ので、新しい版＝NG と判定する。
  ところがこの道具は生の文字列で比べていたので、
    ・NG      … 「第113回  午後問117」（化けた方）
    ・合格    … 「第113回 午後問117」（化けていない古い方）
  となり、入力の source（化けていない）が「合格」に当たって**バッチから外れた**。
  直っていない問題が、二度と流れない状態だった。

  検証と同じ規則（空白を詰める）で突き合わせる。
...
def nkey(s):
    """source を突き合わせるための形（V1.05）。空白を全部詰める。

    仕上げ検証の tidy_src と同じ考え方。生の文字列で比べると、
    同じ問題の「空白が化けた版」と「化けていない版」が別物になる。"""
    ...
    if os.path.exists(xp):
        for ln in io.open(xp, encoding='utf-8').read().split('\n')[1:]:
            ...
            if len(c) >= 3 and c[0].strip():
                exc[nkey(c[0])] = (c[1].strip(), c[2].strip())
...
def emit(base, tmap, skip, harvested):
    ...
    for f in files:
        ...
        for q in d['questions']:
            ...
            if k in tmap:
                ...
                miss['対応表に無い分類'] += 1
            if nkey(q['source']) not in harvested:
                kind = 'new'
    ...
    # V1.06：本数が少ないときは、1本目に捨て駒を置く。
    # PADは各実行の1本目に前回の最後の回答を保存するので、そのままだと
    # 本命が1本も作られない。捨て駒は**本命と同じ内容**にしておく。
    # 別の問題を置くと、その問題の最新版が前回の答えで上書きされてしまう。
    made = sorted(glob.glob(os.path.join(outdir, 'I*.txt')))
    if made and n <= 3 and '--捨て駒なし' not in sys.argv:
        first = made[0]
        no = base_no - 1000            # 本命より必ず若い番号（名前順で先に来る）
        dummy = os.path.join(outdir, 'I%04d.txt' % no)
        if not os.path.exists(dummy):
            io.open(dummy, 'w', encoding='utf-8').write(
                io.open(first, encoding='utf-8').read())
            print('捨て駒を1本置きました：%s（中身は %s と同じ）'
                  % (os.path.basename(dummy), os.path.basename(first)))
            print('  PADは1本目に前回の最後の回答を保存します。1本目は捨てる前提です。')
            n += 1
